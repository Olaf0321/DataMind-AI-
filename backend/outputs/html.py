...
def html_output(text):
    ...
    if match:
        html_data_str = match.group(1).strip()
        
        # Remove leading "svg" keyword if present
        if html_data_str.lower().startswith('html') or html_data_str.lower().startswith('xml'):
            html_data_str = html_data_str[3:].lstrip()

        # Generate filename with timestamp
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f'html_data_{timestamp}.html'
